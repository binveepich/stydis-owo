...
class Scheduler:
    def __init__(self, bot):
        ...
        self.running = False
        self.thread = None
        self._stop_event = Event()  # Thêm Event để dừng an toàn
        ...
        self.gems_check_time = time.time()
        self.stop_time = time.time()
        
        # Thời gian cho các lệnh đặc biệt
    ...
    def _schedule_sleep(self):
        """Lên lịch sleep: active 666-999s, break 333-666s"""
        if self.bot.config.sm != "YES":
            self.is_sleeping = False
            return
        
        active_duration = random.randint(666, 999)
    ...
    def _queue_commands(self):
        ...
        if (self.bot.config.wm == "YES" and 
            self.weapon_counter >= random.randint(2, 4) and
            self.bot.weapons):
    ...
    def stop(self):
        """Dừng scheduler - KHÔNG block luồng chính"""
        self.running = False
